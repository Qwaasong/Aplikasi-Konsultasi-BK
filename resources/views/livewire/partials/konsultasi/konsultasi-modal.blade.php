...
new class extends Component {
    ...
    #[On('create-konsultasi')]
    public function createKonsultasi()
    {
        $this->resetValidation();
        $this->reset(['editingId', 'siswa_id', 'judul', 'jenis_layanan', 'isi_konsultasi', 'tindak_lanjut', 'kategori_id', 'files', 'newFiles', 'existingFiles', 'searchSiswa']);
    ...
    public function save(KasusBkService $service)
    {
        ...
        $this->reset(['editingId', 'siswa_id', 'judul', 'jenis_layanan', 'isi_konsultasi', 'tindak_lanjut', 'kategori_id', 'files', 'existingFiles', 'searchSiswa', 'showStudentModal']);
        $this->tanggal_konsultasi = date('Y-m-d');
        $this->step = 1;
...
<div>
    {{-- Perhatikan nama modalnya sekarang menjadi "form-konsultasi" --}}
    <x-shared.modal name="form-konsultasi" maxWidth="lg">
    <div class="flex flex-col h-full max-h-[80vh]">
        ...
        <div class="bg-bg-light px-6 py-4 border-b border-gray-100 shrink-0">
            <h2 class="text-base font-bold text-gray-900 leading-tight">
                {{ $editingId ? 'Edit Konsultasi' : 'Tambah Konsultasi' }}
            </h2>
            <p class="text-xs text-gray-500 mt-0.5">
                {{ $editingId ? 'Perbarui data layanan konsultasi siswa' : 'Catat layanan konsultasi siswa baru' }}
            </p>
        </div>
        ...
        <div class="px-6 py-4 overflow-y-auto modal-scroll grow" style="scrollbar-width: thin;">
            ...
            <div class="{{ $step === 1 ? 'block' : 'hidden' }}">
                ...
                <div class="mb-5">
                    ...
                    <textarea id="tindak_lanjut" wire:model="tindak_lanjut" rows="4" class="w-full border border-gray-200 rounded-md p-4 text-[14.5px] text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary resize-none shadow-sm leading-relaxed" placeholder="Ceritakan hasil layanan dan langkah tindak lanjut..."></textarea>
                </div>
            </div>
        ...
        <div class="bg-bg-light px-7 py-5 border-t border-gray-100 flex justify-end shrink-0 rounded-b-xl">
            ...
            <div class="{{ $step === 2 ? 'flex' : 'hidden' }} gap-3">
                ...
                <x-atoms.button wire:click="save">
                    {{ $editingId ? 'Perbarui Konsultasi' : 'Simpan Konsultasi' }}
                </x-atoms.button>
            </div>
        </div>
    </div>
