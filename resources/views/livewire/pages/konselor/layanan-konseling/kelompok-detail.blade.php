...
<div class="flex-1 flex flex-col min-w-0 bg-gray-50/50 min-h-screen p-6 lg:p-10">
                    ...
                    @if(strlen($search) >= 2)
                        <div class="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden w-full">
                            @forelse($this->searchResults as $result)
                                <a href="{{ route('konselor.layanan-konseling.kelompok.detail', $result->id) }}" wire:navigate
                                    ...
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $result->siswa->count() }} Peserta &middot; {{ \Carbon\Carbon::parse($result->tanggal_layanan)->format('d M Y') }}</div>
                                </a>
                            @empty
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-teal-50 text-teal-700 border border-teal-100">Layanan Kelompok</span>
                <h1 class="text-2xl font-bold text-gray-900 mt-1">Layanan Konseling Kelompok</h1>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 self-end sm:self-auto">
            ...
            {{-- Delete --}}
            <button wire:click="delete" wire:confirm="Yakin ingin menghapus data layanan kelompok ini?"
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-4">
                    <h3 class="font-bold text-gray-900 text-[15px]">Anggota Kelompok</h3>
                    <span class="px-2.5 py-0.5 text-xs font-bold bg-teal-50 text-teal-700 rounded-full border border-teal-100">
                        {{ $record->siswa->count() }} Siswa
                    </span>
                </div>

                @if($record->siswa->isNotEmpty())
                    <div class="space-y-3 max-h-[360px] overflow-y-auto pr-1" style="scrollbar-width: thin;">
                        @foreach($record->siswa as $peserta)
                            @php
                                $nama = $peserta->siswa->nama_lengkap ?? $peserta->siswa->nama ?? 'Siswa';
                                $words = explode(' ', trim($nama));
                                $initials = count($words) >= 2 
                                    ? strtoupper(substr($words[0], 0, 1) . substr($words[1], 0, 1))
                                    : strtoupper(substr($nama, 0, 2));
                            @endphp
                            <div class="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-teal-100 hover:bg-teal-50/10 transition-all">
                                ...
                                <div class="overflow-hidden">
                                    ...
                                    <p class="text-[11px] text-gray-500 mt-0.5">
                                        NIS {{ $peserta->siswa->nis ?? '-' }} &middot; Kelas {{ $peserta->siswa->kelas_label ?? '-' }}
                                    </p>
                                </div>
                            </div>
                        @endforeach
