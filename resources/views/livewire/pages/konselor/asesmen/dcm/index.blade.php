...
new #[Layout('layouts.app', ['title' => 'Asesmen DCM - Bimbingan Konseling'])] class extends Index {};
...
<div>
    ...
    {{-- Header --}}
    <x-organisms.header action="createDcm">
        <x-slot:search>
        ...
        <div class="px-6 sm:px-8 py-6">
            ...
            <div class="mb-5">
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Pilih tingkat kelas untuk melihat data DCM.
                </p>
    ...
    <x-organisms.data-table
        ...
        @forelse($records as $record)
            ...
            <tr wire:key="dcm-{{ $record->id }}" wire:click="goToDetail({{ $record->id }})"
                ...
                <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap align-middle">
                    {{ optional($record->tanggal)->isoFormat('D MMM Y') }}
                </td>

                <td class="px-4 py-2 font-semibold align-middle">
                    <a
                        href="{{ route('konselor.asesmen.dcm.detail', $record->id) }}"
                        wire:navigate
                ...
                <td class="px-4 py-2 text-right align-middle">
                    <div class="flex items-center justify-end gap-2"
                        ...
                        <x-atoms.action-button
                            color="blue"
                            title="Edit"
                            wire:click="$dispatch('edit-dcm', { id: {{ $record->id }} })">
    ...
    {{-- ═══════════════════════════════════════════ --}}
    @if($showImportModal)
        <x-shared.modal name="import-dcm" :show="true" model="showImportModal" maxWidth="md">
            <div class="flex flex-col">
                <div class="bg-bg-light px-6 py-4 border-b border-gray-100 shrink-0">
                    <h2 class="text-base font-bold text-gray-900">Import Data DCM</h2>
                ...
                <div class="px-6 py-5 space-y-4">
                    <div class="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs text-blue-700">
                        ...
                        <code class="block">nis | tanggal | jawaban | kesimpulan | catatan</code>
                        <p class="mt-1 text-blue-500">Kolom jawaban berisi kode masalah dipisah koma (mis. "A01, C04"). NIS + tanggal adalah kunci; import ulang akan memperbarui data yang sama.</p>
                    </div>
    ...
    {{-- ═══════════════════════════════════════════ --}}
    @if($showExportModal)
        <x-shared.modal name="export-dcm" :show="true" model="showExportModal" maxWidth="md">
            <div class="flex flex-col">
                <div class="bg-bg-light px-6 py-4 border-b border-gray-100 shrink-0">
                    <h2 class="text-base font-bold text-gray-900">Export Data DCM</h2>
    ...
    {{-- Modal DCM --}}
    @include('livewire.partials.asesmen.dcm.dcm-modal', [
        'editingId' => $editingId
    ])
