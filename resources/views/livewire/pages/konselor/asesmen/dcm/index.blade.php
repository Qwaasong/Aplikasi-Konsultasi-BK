...
<div>
    ...
    <x-organisms.data-table
        :headers="['', 'Tanggal', 'Siswa', 'Masalah Teridentifikasi', 'Kesimpulan', 'Aksi']"
        empty="Belum ada data DCM.">

        @forelse($records as $record)

            <tr wire:key="dcm-{{ $record->id }}" wire:click="goToDetail({{ $record->id }})"
                class="group border-b border-gray-100 transition-all duration-200 h-12">
                ...
                <td class="w-16 text-center align-middle">
                    <input
                        ...
                        class="w-4 h-4 rounded border-gray-300 text-brand-teal focus:ring-brand-teal">
                </td>
                ...
                <td class="px-4 py-2 text-sm text-gray-600">
                    {{ is_array($record->masalah_teridentifikasi)
                        ? count($record->masalah_teridentifikasi) . ' masalah'
                        : '-' }}
                </td>
