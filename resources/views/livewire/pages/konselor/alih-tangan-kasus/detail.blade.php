...
use App\Livewire\Konselor\Bk\AlihTanganKasus\Detail;
...
<div class="flex-1 flex flex-col min-w-0 bg-gray-50/50 min-h-screen p-6 lg:p-10">
                    ...
                    @if(strlen($search) >= 2)
                        <div class="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden w-full">
                            @forelse($this->searchResults as $result)
                                <a href="{{ route('konselor.alih-tangan-kasus.detail', $result->id) }}" wire:navigate
                                    class="block px-4 py-3 hover:bg-gray-50 border-b border-gray-100 last:border-0 transition-colors">
                                    <div class="text-sm font-semibold text-gray-800 truncate">{{ $result->kasus?->siswa?->nama ?? '-' }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $result->tanggal_alih ? \Carbon\Carbon::parse($result->tanggal_alih)->format('d M Y') : '-' }}</div>
                                </a>
                            @empty
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-50 text-orange-700 border border-orange-100">Alih Tangan Kasus</span>
                <h1 class="text-2xl font-bold text-gray-900 mt-1">{{ $record->kasus?->siswa?->nama ?? '-' }}</h1>
                <p class="text-sm text-gray-500 mt-0.5">NIS {{ $record->kasus?->siswa?->nis ?? '-' }} &middot; Kelas {{ $record->kasus?->siswa?->kelas_label ?? '-' }} - {{ $record->kasus?->siswa?->jurusan_label ?? '-' }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 self-end sm:self-auto">
            ...
            <button wire:click="delete" wire:confirm="Yakin ingin menghapus data alih tangan kasus ini?"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                <div class="flex items-center gap-3 border-b border-gray-100 pb-4 mb-4">
                    ...
                    <h3 class="font-bold text-gray-900 text-[15px]">Alasan Alih Tangan</h3>
                </div>
                <p class="text-sm leading-relaxed text-gray-700 whitespace-pre-line text-justify">
                    {{ $record->alasan_alih ?: 'Tidak ada alasan yang dicantumkan.' }}
                </p>
            </div>
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                ...
                <p class="text-sm leading-relaxed text-gray-700 whitespace-pre-line text-justify">
                    {{ $record->tindak_lanjut ?: 'Belum ada tindak lanjut.' }}
                </p>
            </div>
        ...
        <div class="space-y-6">
            ...
            {{-- Informasi Alih Tangan --}}
            <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                <h3 class="font-bold text-gray-900 text-[15px] border-b border-gray-100 pb-4 mb-4">Informasi Alih Tangan</h3>
                <div class="space-y-4">
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-gray-500">Tanggal Alih Tangan</span>
                        <span class="font-semibold text-gray-800">
                            {{ $record->tanggal_alih ? \Carbon\Carbon::parse($record->tanggal_alih)->locale('id')->translatedFormat('d F Y') : '-' }}
                        </span>
                    </div>
                    <div class="border-t border-gray-50 pt-4 mt-2">
                        <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Dari Guru BK</span>
                        <div class="bg-gray-50 p-3 rounded-xl border border-gray-100">
                            <p class="text-sm font-semibold text-gray-800">{{ $record->guruBkAsal?->user?->nama ?? '-' }}</p>
                            <p class="text-[11px] text-gray-500 mt-0.5">NIP {{ $record->guruBkAsal?->nip ?? '-' }}</p>
                        </div>
                    </div>
                    <div class="border-t border-gray-50 pt-4 mt-2">
                        <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Diterima Oleh</span>
                        <div class="bg-gray-50 p-3 rounded-xl border border-gray-100">
                            <p class="text-sm font-semibold text-gray-800">{{ $record->guruBkTujuan?->user?->nama ?? '-' }}</p>
                            <p class="text-[11px] text-gray-500 mt-0.5">NIP {{ $record->guruBkTujuan?->nip ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                <div class="flex items-center gap-3 border-b border-gray-100 pb-4 mb-4">
                    ...
                    <h3 class="font-bold text-gray-900 text-[15px]">Kasus Terkait</h3>
                </div>
                <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-2">
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-gray-500">Penanganan</span>
                        <span class="font-semibold text-gray-800 text-right max-w-[200px] truncate">{{ $record->kasus?->penanganan ?? '-' }}</span>
                    </div>
                    <div class="border-t border-gray-50 pt-2">
                        <span class="text-gray-500">Prioritas</span>
                        <span class="font-semibold text-gray-800">{{ $record->kasus?->prioritas ?? '-' }}</span>
                    </div>
                    <div class="border-t border-gray-50 pt-2">
                        <span class="text-gray-500">Status</span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                            @if($record->kasus?->status === 'Open')
                                bg-green-100 text-green-700
                            @elseif($record->kasus?->status === 'Pending')
                                bg-yellow-100 text-yellow-700
                            @elseif($record->kasus?->status === 'Closed')
