...
use App\Livewire\Konselor\Bk\AlihTanganKasus\Index;
...
<div class="flex-1 flex flex-col min-w-0 bg-white h-full" x-data="{ loading: false }"
    ...
    @if($showFilters)
        <div class="px-6 sm:px-8 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-4 text-sm text-gray-600 shrink-0 transition-all">
            ...
            <select wire:model.live="filterKelas" class="text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-brand-teal w-28 sm:w-36 pr-8 flex-shrink-0 bg-white cursor-pointer">
                <option value="">Semua Kelas</option>
                @foreach($kelasOptions ?? [] as $k)
                    <option value="{{ $k }}">Kelas {{ $k }}</option>
                @endforeach
            </select>
            <select wire:model.live="filterJurusan" class="text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-brand-teal w-32 pr-8 flex-shrink-0 bg-white cursor-pointer">
            ...
            <input type="date" wire:model.live="filterTanggal"
                class="text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-brand-teal bg-white cursor-pointer">
            @if($search !== '' || $filterKelas !== '' || $filterJurusan !== '' || $filterTanggal !== '')
    ...
    <x-organisms.data-table
        :headers="['', 'Tanggal', 'Siswa', 'Guru Asal', 'Guru Tujuan', 'Alasan', 'Aksi']"
        ...
        @forelse($records as $record)
            <tr wire:key="atk-{{ $record->id }}"
                ...
                <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap align-middle">
                    {{ \Carbon\Carbon::parse($record->tanggal_alih)->isoFormat('D MMM Y') }}
                </td>
                ...
                <td class="px-4 py-2 text-sm text-gray-600 align-middle">
                    {{ $record->guruBkAsal?->user?->nama ?? '-' }}
                </td>

                <td class="px-4 py-2 text-sm text-gray-600 align-middle">
                    {{ $record->guruBkTujuan?->user?->nama ?? '-' }}
                </td>
                ...
                <td class="px-4 py-2 text-right align-middle">
                    <div class="flex items-center justify-end gap-2" onclick="event.stopPropagation()">
                        ...
                        <x-atoms.action-button color="red" title="Hapus" wire:click="delete({{ $record->id }})"
                            wire:confirm="Yakin ingin menghapus alih tangan kasus ini?">
