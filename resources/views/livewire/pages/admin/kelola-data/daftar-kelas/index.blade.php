    ...
    public function with(): array
    {
        ...
        if ($this->search) {
            ...
            $data = $data->filter(function ($item) use ($needle) {
                ...
                return
                    str_contains(strtolower($item->nama_kelas ?? ''), $needle)
        ...
        if ($this->filterSekolah) {
            $data = $data->filter(fn($item) =>
                $item->jurusan?->sekolah?->nama_sekolah === $this->filterSekolah
            );
        }

        if ($this->filterJurusan) {
            $data = $data->filter(fn($item) =>
                $item->jurusan?->nama_jurusan === $this->filterJurusan
            );
        }

        if ($this->filterTingkat) {
            $data = $data->filter(fn($item) =>
                $item->tingkat === $this->filterTingkat
            );
        }
    ...
    @if($showFilters)
        <div class="px-6 sm:px-8 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-4 text-sm text-gray-600">
            ...
            <select wire:model.live="filterSekolah"
                class="text-xs border border-gray-200 rounded px-2 py-1.5 w-44 bg-white">
            ...
            @if($search || $filterSekolah || $filterJurusan || $filterTingkat)
...
<tr
    ...
    <td class="px-4 py-2 text-xs">
        ...
        <span class="px-2 py-1 rounded-full bg-green-100 text-green-700 font-medium">
            ...
            {{ $record->jurusan?->sekolah?->nama_sekolah }}
    ...
    <td class="px-4 py-2 text-xs">
        ...
        <span class="px-2 py-1 rounded-full bg-purple-100 text-purple-700 font-medium">

            {{ $record->waliKelas?->user?->nama ?? '-' }}
...
@endforeach
    ...
    <livewire:partials.admin.kelola-data.daftar-kelas.daftar-kelas-modal />
