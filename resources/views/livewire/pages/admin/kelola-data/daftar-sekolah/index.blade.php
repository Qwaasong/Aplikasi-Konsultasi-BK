    ...
    public function with(): array
    {
        ...
        if ($this->search) {
            ...
            $data = $data->filter(function ($item) use ($needle) {
                ...
                return
                    str_contains(strtolower($item->nama_sekolah ?? ''), $needle) ||
...
<div class="flex-1 flex flex-col min-w-0 bg-white h-full" x-data="{ loading: false }"
    ...
    <div class="px-6 sm:px-8 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-4 text-sm text-gray-600">
        ...
        <select
            ...
            class="text-xs border border-gray-200 rounded px-2 py-1.5 w-56 bg-white">
    ...
    empty="Belum ada data sekolah.">

    @foreach($records as $record)

    <tr
        wire:key="sekolah-{{ $record->id }}"
        ...
        <td class="w-16 text-center">
            <input
                ...
                class="w-4 h-4 rounded border-gray-300 text-brand-teal">
        </td>
        ...
        <td class="px-4 py-2">
            ...
            @else
                <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-xs text-gray-400">
                    -
                </div>
            @endif
        </td>
...
@endforeach
    ...
    <livewire:partials.admin.kelola-data.daftar-sekolah.daftar-sekolah-modal />
